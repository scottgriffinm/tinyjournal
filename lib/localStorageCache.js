...
export function updateCache(key, newItem, ttlMs = 5 * 60 * 1000) {
    ...
    if (cachedString) {
        try {
            ...
            // If the cache is expired, initialize a new cache
            if (!expiry || Date.now() > expiry) {
                console.log("Cache expired, resetting with new item for key:", key);
                cachedData = key === "journalEntries" ? [] : {}; // Reset to list or object based on key
            } else {
                cachedData = data; // Use existing cached data
            }
        } catch (error) {
            console.error("Error parsing existing cache, resetting with new item for key:", key, error);
            cachedData = key === "journalEntries" ? [] : {}; // Reset to list or object on error
        }
    } else {
        console.log("No cache found, initializing new cache for key:", key);
        cachedData = key === "journalEntries" ? [] : {}; // Initialize new cache if none exists
    }
